...
export async function initMMG3D(): Promise<void> {
  ...
  module = (await createModule()) as unknown as MMG3DModule;
}
...
export const MMG3D = {
   ...
   * @param handle - The mesh handle
   * @param typEntity - Entity type (use SOL_ENTITY.VERTEX)
   * @param nEntities - Number of entities (typically number of vertices)
   * @param typSol - Solution type (use SOL_TYPE.SCALAR or SOL_TYPE.TENSOR)
   * @throws Error if allocation fails
   */
  setSolSize(
    handle: MeshHandle,
    ...
    const m = getModule();
    const result = m._mmg3d_set_sol_size(handle, typEntity, nEntities, typSol);
    if (result !== 1) {
   ...
   */
  getSolSize(handle: MeshHandle): SolInfo {
    const m = getModule();
    ...
    try {
      const result = m._mmg3d_get_sol_size(handle, ptr, ptr + 4, ptr + 8);
      if (result !== 1) {
   ...
   * Must call setSolSize with SOL_TYPE.SCALAR first.
   * @param handle - The mesh handle
   * @param values - Float64Array of solution values (one per vertex)
   * @throws Error if setting fails
   */
  setScalarSols(handle: MeshHandle, values: Float64Array): void {
    const m = getModule();
    ...
    try {
      ...
      const result = m._mmg3d_set_scalar_sols(handle, valuesPtr);
      if (result !== 1) {
   ...
   */
  getScalarSols(handle: MeshHandle): Float64Array {
    const m = getModule();
    ...
    try {
      const dataPtr = m._mmg3d_get_scalar_sols(handle, countPtr);
      if (dataPtr === 0) {
        throw new Error(
   ...
   * Must call setSolSize with SOL_TYPE.TENSOR first.
   * @param handle - The mesh handle
   * @param values - Float64Array of tensor values (6 components per vertex: m11, m12, m13, m22, m23, m33)
   * @throws Error if setting fails
   */
  setTensorSols(handle: MeshHandle, values: Float64Array): void {
    const m = getModule();
    ...
    try {
      m.HEAPF64.set(values, valuesPtr / 8);
      const result = m._mmg3d_set_tensor_sols(handle, valuesPtr);
      if (result !== 1) {
   ...
   */
  getTensorSols(handle: MeshHandle): Float64Array {
    const m = getModule();
    ...
    try {
      const dataPtr = m._mmg3d_get_tensor_sols(handle, countPtr);
      if (dataPtr === 0) {
        throw new Error(
      ...
      try {
        const count = m.getValue(countPtr, "i32");
        // 6 components per vertex for 3D tensor
